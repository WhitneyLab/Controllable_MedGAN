...
import cv2
def create_from_images(tfrecord_dir, image_dir, shuffle=True):
    print('Loading images from "%s"' % image_dir)
    # image_filenames = glob.glob(os.path.join(image_dir, '*')) # Original
    image_filenames = glob.glob(os.path.join(image_dir, 'ham10000_images_part_1/*')) + glob.glob(os.path.join(image_dir, 'ham10000_images_part_2/*'))
    # image_filenames_full = []
    # for folder_name in glob.glob(os.path.join(image_dir, '*')):
    #     image_filenames_full += [folder_name + '/' + filepath for filepath in os.listdir(folder_name)]
    # image_filenames = sample(image_filenames_full,20000)
    # image_filenames = sample(image_filenames,20000)
    ...
    img = np.asarray(PIL.Image.open(image_filenames[0]).resize((256, 256)))
