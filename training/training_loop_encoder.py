...
def training_loop(
    ...
    with tf.device('/gpu:0'):
        if resume_run_id is not None:
            ...
            # G = tflib.Network('G', num_channels=3, resolution=256, label_size=0, **G_args)
            # D = tflib.Network('D', num_channels=3, resolution=256, label_size=0, **D_args)
            # Gs = G.clone('Gs')
    ...
    init_new_vars_op = tf.initialize_variables([global_step0])
    ...
    for it in range(start, max_iters):
        ...
        if cur_nimg >= tick_start_nimg + 10000: #65000
            cur_tick += 1
            tick_start_nimg = cur_nimg
