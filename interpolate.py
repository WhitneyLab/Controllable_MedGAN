...
def parse_args():
  ...
  #                     help='Path to the pre-trained model.') # Breast Model
  ...
  parser.add_argument('-src_dir', type=str, default='/home/zhren/BreastGAN/idinvert/results/inversion/mooney_test/',
                      help='Source directory, which includes original images, '
                           'inverted codes, and image list.')
  parser.add_argument('-dst_dir', type=str, default='/home/zhren/BreastGAN/idinvert/results/inversion/mooney_test/',
...
def main():
  ...
  print(src_codes.shape)

  dst_codes = np.zeros(src_codes.shape)
  dst_codes[0:2,:] = src_codes[1:,:]
  dst_codes[-1,:] = src_codes[0,:]
  dst_list = src_list[1:]
  ...
  for src_idx in tqdm(range(num_src), leave=False):
    ...
    for dst_idx in tqdm(range(num_dst), leave=False):
      ...
      os.mkdir('./results/interpolation/' + str(src_idx) + '_' + str(dst_idx) + '/')
      for s, output_image in enumerate(output_images):
        cv2.imwrite('./results/interpolation/' + str(src_idx) + '_' + str(dst_idx) + '/' + str(s+1) + '.jpg', output_image)
